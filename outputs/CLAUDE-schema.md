...
**你不能动**：
- raw/ ——只读的事实来源；唯一编辑机会是录入窗口期（§6④），摄取即冻结。来源真变了走重新摄取（新快照），不原地改
...
raw/ 的**分类原则**固定（按数据源的安全边界与摄取管线分类）；**具体子目录是示例，按需增删**。wiki/ 结构只是初始建议——你根据实际需求自由重构（例如 entity/ 下可拆为 entity/person/、entity/org/ 等）。

**四层职责**：raw/（不可变来源）· wiki/（提炼的知识）· 工作区（vault 外、经 workspaces.md 登记的项目目录）· 本文件（宪法）。工作区**不放进 vault**——放 vault 外任意路径，逐个登记进 workspaces.md。raw/ 只放不可变的外部快照与冻结参考件。工作区隔离原理详见 §16。
...
工作区是"活的"目录——人和你共同干活、产出成品，与 raw/（只读来源）、wiki/（提炼知识）分属不同层。结构参考 IPOF（来自 Second Brain Guide，非 llm-wiki 原典）：
